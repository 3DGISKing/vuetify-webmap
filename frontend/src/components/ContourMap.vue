...
const createContours = (data, options) => {
    ...
    var contours = d3contours().size([data.z[0].length, data.z.length]).thresholds(thresholds)(values);
        ...
        for (var j = 0; j < contours[i].coordinates.length; j++) {
            for (var k = 0; k < contours[i].coordinates[j].length; k++) {
                for (var l = 0; l < contours[i].coordinates[j][k].length; l++) {
                    contours[i].coordinates[j][k][l] = unitToGeographic(
                        data.x,
                        data.y,
...
onMounted(() => {
    ...
    const contour = createContours(data, {
        thresholds: 50
    });
    ...
    contour.forEach((contourData) => {
        var myStyle = {
            color: getColor(contourData.value, 0.01, 6.46, colors),
            opacity: 1,
            weight: 2,
            fillOpacity: 0
        };
